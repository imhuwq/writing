...
- 程序加密
---
AI 模型商用场景下，在部署时会遇到两个常见问题：
- 技术方：保护技术知识产权
- 应用方：保护数据安全隐私

把 AI 程序加密后私有化部署到应用方，是一个可行的方案。  
但是，AI 技术方付出了更多的妥协，需要花费更多的精力做好程序加密。

AI 程序一般至少包含 AI 模型和推理逻辑。如果是加密模型，则还要加上解密逻辑。
```
AI 程序 = AI 模型 + 推理逻辑
AI 程序 = 加密模型 + 解密逻辑 + 推理逻辑
AI 程序 = 加密模型 + 加密“解密逻辑” + (加密)推理逻辑
```
需要注意的是，**如果模型加密了，就必须把解密逻辑也加密**，否则就是多此一举。
<!-- more -->

```
那进一步地，又如何解密“加密解密逻辑”呢？  
这是一个可以无限追溯的问题，无法解决。
因为不管这个步骤有多少层，最后交付的程序想要运行起来，就必须对机器透明。
所以理论上没有不能破解的程序。
当然，程序对机器透明，但是机器对 Hacker 不一定透明。
我们防破解的所有工作，都在于努力扩大这间隙：提高机器码的理解成本、或者干脆隔离机器/硬件。
```

## 1. 破解程序的常见思路
破解程序只是手段，而不是最终目的。最终目的是：
- 目的 1: 拿到解密后的模型(相当于可以进行二次开发)
- 目的 2: 实现无限制的算法调用(相当于可以免费使用)
...
客户端在进行推理之前，客户端必须解密模型。
但是解密模型的 key 和逻辑，也必须以某种形式交付给客户端进行部署。

这是最大的漏洞。如果把模型用最先进的技术加密，但是用明文的方式写上了解密步骤，就和“**把钱存到银行，但是把密码写到银行卡背面**”是一样的。  
...
这个太简单了，不详细展开。  
```
顺带说一下，经过测试，Python 3.8 及以下版本可以使用常用的 uncompyle6 进行逆向，Python 3.9 及以上的能用 pycdc(https://github.com/zrax/pycdc) 工具进行逆向。
```
...
但是，正如之前所说的，破解程序不是目的，只是手段。很多时候**不需要破解**程序代码或者逻辑，**只需要拦截**一些关键 API 的调用就可以了。

比如说，大部分加密/解密程序都用到了 `cryptography` 依赖包，大部分 AI 程序都用到了 `torch`，那我只要拦截他们的关键 `API` 即可。  
...
举个比上面 `hack_crypto` 更恐怖的例子。绝大部分 AI 程序都用到了 `torch` 框架。
...
PyArmor 加密不可逆、不可分析汇编指令(✨✨✨保护)，但是不能防止非法调用。
C++ 编译不可逆、可以分析汇编指令的(✨✨保护)，并且可以提高非法调用的门槛。
...
本文多次提到防止拦截的手段是编译私有 `so` 动态链接库。但其实  `so` 的逆向并非完全不可行。这篇文章之所以这么强调 `so` 的安全性，主要想表达的意思是：**在 Python 内做程序加密意义不大，还是得依靠编译**。
...
因为 Hacker 破解程序的意图强度，只取决于投入和回报的比例。
