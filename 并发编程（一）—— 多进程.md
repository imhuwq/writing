 ...
 */
```
正如上述程序所展示的那样，`fork` 返回了两次，两个进程共享了内存(`g`)，但是修改内存时触发了写时复制(`l`)。子进程复制了父进程的文件描述符表，所以它们的 `printf` 输出到同一个 `STDOUT`。  
```c
// exec_example.c
#include <sys/types.h>
#include <unistd.h>
 ...
 */
```  
上述程序在子进程中使用 `exec` 把子进程替换为 `/bin/ls` 程序，替换成功后，子进程就不再执行了。但是因为 `exec` 不会删除文件描述符表，所以其 STDOUT 和父进程还是同一个。  
 ...
 */
```
在上述程序中，程序被打断后，触发 `SIGINT` 处理函数，在其中又使用 `kill` 系统调用向自己发送了 `SIGABRT` 信号。从 `kill` 系统调用返回后，内核检查发现接受到了 `SIGABRT` 信号，于是没有接着执行 `SIGINT` 的处理函数，而是先去执行 `SIGABRT` 的处理函数。当 `SIGABRT` 处理函数返回后，没有检查到新的信号，于是继续执行 `SIGINT` 处理函数。  
